...
describe('Aggregator Processor', () => {
  ...
  test('createOrder', async () => {
    ...
    Aggregator.prototype.createOrder = jest
      .fn()
      .mockResolvedValue(expectedResponse.redirect_url);

    const response = await aggregatorProcessor.createOrder(requestPayload);

    expect(response).toEqual(expectedResponse);
    expect(Aggregator.prototype.createOrder).toHaveBeenCalledWith(
      requestPayload
    );
  });
  ...
  test('getPaymentDetails', async () => {
    ...
    const response = await aggregatorProcessor.getPaymentDetails({
      gid: 'TR8273429127187',
    });
  ...
  test('createRefund', async () => {
    const requestPayload = {
      ...
      meta: {
        ...
        transaction_ids: ['TR67160B990EA2149E59'],
      },
      company_id: 1987,
    ...
    Aggregator.prototype.createRefund = jest
      .fn()
      .mockResolvedValue(refundResponse);
