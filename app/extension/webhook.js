...
class WebhookRegistry {
    ...
    async initialize(config, fdkConfig) {
        ...
        if (!config.notification_email || !emailRegex.test(config.notification_email)) {
            throw new InvalidWebhookConfig(`Invalid or missing "notification_email"`);
        }
        if (!config.api_path || config.api_path[0] !== '/') {
            throw new InvalidWebhookConfig(`Invalid or missing "api_path"`);
        }
        if (!config.event_map || !Object.keys(config.event_map).length) {
            throw new InvalidWebhookConfig(`Invalid or missing "event_map"`);
        }
        ...
        if(Object.keys(eventConfig.eventsNotFound).length){
            ...
            throw new InvalidWebhookConfig(`Webhooks events ${errors.join(' and ')} not found`);
        }
    ...
    async syncEvents(platformClient, config = null, enableWebhooks) {
        ...
        try {
            ...
            throw new WebhookRegistrationError(`Failed to sync webhook events. Reason: ${ex.message}`);
        }
    }
    ...
    async enableSalesChannelWebhook(platformClient, applicationId) {
        ...
        if (this._config.subscribed_saleschannel !== 'specific') {
            throw new WebhookRegistrationError('`subscribed_saleschannel` is not set to `specific` in webhook config');
        }
        try {
            let subscriberConfig = await this.getSubscriberConfig(platformClient);
            if (!subscriberConfig) {
                throw new WebhookRegistrationError(`Subscriber config not found`);
            }
            ...
            throw new WebhookRegistrationError(`Failed to add saleschannel webhook. Reason: ${ex.message}`);
        }
    }
    ...
    async disableSalesChannelWebhook(platformClient, applicationId) {
        ...
        try {
            ...
            throw new WebhookRegistrationError(`Failed to remove saleschannel webhook. Reason: ${ex.message}`);
        }
    }
    ...
    async processWebhook(req) {
        ...
        try {
            ...
            if (typeof extHandler === 'function') {
                ...
                throw new WebhookHandlerNotFound(`Webhook handler not assigned: ${categoryEventName}`);
            }
        }
        catch (err) {
            throw new WebhookProcessError(err.message);
        }
    }
    ...
    async getSubscriberConfig(platformClient) {
        try {
            ...
            throw new InvalidWebhookConfig(`Error while fetching webhook subscriber configuration, Reason: ${err.message}`)
        }
    }
        ...
        try {
            ...
            Object.keys(handlerConfig).forEach((key) => {
                ...
                if (eventDetails.length !== 3) {
                    throw new InvalidWebhookConfig(`Invalid webhook event map key. Invalid key: ${key}`)
                }
            ...
            throw new InvalidWebhookConfig(`Error while fetching webhook events configuration, Reason: ${err.message}`)
        }
    }
}
